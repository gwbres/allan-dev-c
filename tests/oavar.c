...
int main (int argc, char **argv)
{
	...
	oavar (x, y, N, AVAR_FREQ_DATA, axis);
	
	if (axis == TAU_AXIS_POW2)
		array2csv ("oavar-freq.csv", y, (int)log2(N)-1);
	else if (axis == TAU_AXIS_POW10)
		array2csv ("oavar-freq.csv", y, (int)log10(N)-1);
	
	printf("######### AVAR ('phase' data) ###########\n");
	oavar (x, y, N, AVAR_PHASE_DATA, axis);

	if (axis == TAU_AXIS_POW2)
		array2csv ("oavar-phase.csv", y, (int)log2(N)-1);
	else if (axis == TAU_AXIS_POW10)
		array2csv ("oavar-phase.csv", y, (int)log10(N)-1);
